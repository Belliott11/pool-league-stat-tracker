...
instead of resize.** The first attempt leaned entirely on `align-items: stretch` (the row's
cross-size is the *taller* panel's own natural content height, and both stretch to match it) plus
making `.panel:has(> #awardsVsStats)` (the `>` matters — `#awardsVsStats` is a direct child of that
`.panel`) a `display: flex; flex-direction: column`, with `#awardsVsStats` itself as the flexible,
scrolling child (`flex: 1 1 auto; min-height: 0; overflow-y: auto`). That only actually engages the
scroll when this panel happens to be the *shorter* one, though — stretch expands every item in a
row to match whichever is tallest, so if Awards vs. Stats' own natural content is the taller one
(11 real awards, several multi-winner, comfortably is), the row just stretches Power Ranking vs.
Performance up to match it instead, and Awards never clips at all. Fixed with a flat
`max-height: 500px` added directly on `#awardsVsStats` itself, on top of the existing flex rules —
decoupled entirely from whatever height the row's stretch computes, so scrolling kicks in past a
consistent, predictable point regardless of which panel happens to be taller on any given render.
`min-height: 0` still matters for the same reason as before: without it, a flex child refuses to
shrink below its own content size and the scroll never kicks in even with `max-height` set.
