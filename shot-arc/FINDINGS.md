...
## Ben labeled `real_5gqbi2wxew52g5p_1746_246`: "no shot happens here" -- the shot is ~3s AFTER the logged videoTime, and a shifted window fits cleanly

The label export had the whole window (1-120) as its range, i.e. nothing marked, so it was filed under `labels/rejected/` rather than used as a fit range. A 1-frame-per-second contact sheet from 6s before to 8s after the logged time (4K source) shows the ball only in flight toward the hoop about 3s after `videoTime`, well outside the current 3.0s-before/1.0s-after window.
